...
class Container(containers.DeclarativeContainer):
    ...
    sqlite_database = providers.Singleton(
        Database, db_url=config.sqlite_url, database_factory=providers.Factory(SQLiteDatabaseFactory)
    )

    postgres_database = providers.Singleton(
        Database, db_url=config.postgres_url, database_factory=providers.Factory(PostgresDatabaseFactory)
    )
