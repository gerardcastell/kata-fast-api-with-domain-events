...
async def dispatch_batch_tasks(
    ...
    try:
        task_ids = await task_dispatcher.dispatch_batch_tasks(
            tasks=request.tasks,
            priority=request.priority,
            delay_seconds=request.delay_seconds,
            max_retries=request.max_retries,
...
@inject
async def get_queue_status(sqs_client: SQSClient = Depends(Provide[Container.sqs_client])):
...
async def dispatch_data_processing_task(
    ...
    return await task_dispatcher.dispatch_task(
...
async def dispatch_email_notification_task(
    ...
    return await task_dispatcher.dispatch_task(request.task_type, request.payload)
